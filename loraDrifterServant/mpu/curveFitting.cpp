...
void printMat(const char * s, const double * m, const int n) {
  Serial.println(s);
  char buf[40];
  for(int i = 0; i < n; i++) {
    for(int j = 0; j < n; j++) {
...
//Raise x to power
double curveFitPower(const double base, const int exponent){
  if(exponent == 0) {
    return 1;
  }
  double val = base;
...
int fitCurve(int order, int nPoints, double py[], int nCoeffs, double * coeffs) {
  ...
  if(nCoeffs > MAX_ORDER || nCoeffs < 2) return ORDER_INCORRECT; 	//matrix memory hard coded for max of 20 order, which is huge
  ...
  for(int i = 0; i < nPoints; i++) { //Generate matrix elements
    const double x = px[i];
    const double y = py[i];
    for(int j = 0; j < (nCoeffs * 2) - 1; j++) {
  ...
  for(int i = 0; i < nCoeffs; i++) {//index by matrix row each time
    for(int j = 0; j < nCoeffs; j++) {//index within each row
...
//   cpyArray(masterMat, mat, nCoeffs);
  memcpy(mat, masterMat, nCoeffs * nCoeffs);
  const double denom = det(mat, nCoeffs, CURVE_FIT_DEBUG); //denominator for Cramer's rule, determinant of LHS linear equation
//   cpyArray(masterMat, mat, nCoeffs);
  memcpy(mat, masterMat, nCoeffs * nCoeffs);

  //Generate cramers rule mats
  for(int i = 0; i < nCoeffs; i++){ //Temporary matrix to substitute RHS of linear equation as per Cramer's rule
